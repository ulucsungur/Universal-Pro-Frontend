...
// 1. TİP TANIMLAMALARI (TS Hatalarını Bitirir)
interface Category {
  id: number;
  title: string;
  imageUrl?: string;
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans">
      <header className="h-20 border-b border-white/5 flex items-center px-10 justify-between sticky top-0 bg-[#020617]/80 backdrop-blur-xl z-50">
        ...
        <button className="text-[10px] font-black uppercase tracking-widest border border-white/20 px-6 py-2 rounded-full hover:bg-white hover:text-black transition-all">
          Giriş Yap
        </button>
      </header>

      <main className="max-w-[1400px] mx-auto p-10 space-y-20">
        {/* KATEGORİLER - Tailwind v4 Standardı (bg-linear-to-t) */}
        <section>
            ...
            {categories.map((cat) => (
              <div key={cat.id} className="group cursor-pointer">
                <div className="relative aspect-[4/5] overflow-hidden rounded-3xl border border-white/5">
                  ...
                  />
                  {/* Tailwind v4 Yeni Sınıf: bg-linear-to-t */}
